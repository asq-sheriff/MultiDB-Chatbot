...
class MultiDatabaseService:
    ...
    async def process_user_message_with_auth(self, session_id: str, user_message: str,
        ...
        await self.conversation_history.save_message(session_uuid, 'bot', response['message'])
    ...
    async def start_background_task_with_auth(self, user_token: str, task_type: str,
        ...
        user = await self.auth_service.get_user_by_id(uuid.UUID(token_payload["user_id"]))
    ...
    async def _check_background_task_quota(self, user: User) -> None:
        ...
        quota_info = await self.billing_service.check_user_quota(user, "background_tasks")
...
def _normalize_faq_row(item: Any) -> Dict[str, Any]:
    ...
    if isinstance(item, dict):
        ...
        scylla_key = getattr(item, "scylla_key", None) or getattr(item, "id", None) or getattr(item, "key", None)
        ...
        updated_at = getattr(item, "updated_at", None) or getattr(item, "modified_at", None) or getattr(item, "ts",
                                                                                                        None)
