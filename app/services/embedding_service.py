...
class EmbeddingConfig:
    ...
    fallback_model: str = os.getenv("EMBEDDING_FALLBACK_MODEL", "sentence-transformers/all-mpnet-base-v2")
    ...
    max_cache_size_mb: int = int(os.getenv("EMBEDDING_CACHE_MB", "1024"))  # Reduced for MacBook
    thread_pool_workers: int = int(os.getenv("EMBEDDING_THREADS", "1"))    # Single thread for stability
    memory_cleanup_threshold: float = float(os.getenv("MEMORY_CLEANUP_THRESHOLD", "0.80"))  # 80% RAM usage
...
class EmbeddingService:
    ...
    async def embed_query(self, text: str) -> List[float]:
        ...
        try:
            ...
            result = await asyncio.wait_for(
                loop.run_in_executor(self._thread_pool, self._embed_single, text.strip()),
            ...
            if elapsed > 1.0:  # Log slow queries
                logger.warning(f"Slow all-mpnet-base-v2 query: {elapsed:.2f}s for {len(text)} chars")

            return result

        except asyncio.TimeoutError:
            logger.error(f"all-mpnet-base-v2 embedding timeout after {self.config.query_timeout_seconds}s")
            raise TimeoutError(f"Embedding timeout: {self.config.query_timeout_seconds}s")
        except Exception as e:
            logger.error(f"all-mpnet-base-v2 embedding failed: {e}")
            raise RuntimeError(f"Embedding failed: {e}")

    async def embed_batch(self, texts: List[str], show_progress: bool = True) -> List[List[float]]:
        """
        ...
        try:
            loop = asyncio.get_event_loop()
            result = await asyncio.wait_for(
                loop.run_in_executor(self._thread_pool, self._embed_batch_sync, clean_texts, show_progress),
            ...
            if show_progress:
                logger.info(f"all-mpnet-base-v2 batch embedding: {len(clean_texts)} texts in {elapsed:.1f}s ({rate:.1f} texts/sec)")

            return result

        except asyncio.TimeoutError:
            logger.error(f"all-mpnet-base-v2 batch timeout after {self.config.batch_timeout_seconds}s")
            raise TimeoutError(f"Batch embedding timeout: {self.config.batch_timeout_seconds}s")
        except Exception as e:
    ...
    def _embed_batch_sync(self, texts: List[str], show_progress: bool) -> List[List[float]]:
            ...
            with torch.no_grad():  # Prevent gradient computation
                for i in range(0, len(valid_texts), effective_batch_size):
                    ...
                    if show_progress and len(valid_texts) > effective_batch_size:
                        ...
                        total_batches = (len(valid_texts) - 1) // effective_batch_size + 1
                        logger.info(f"all-mpnet-base-v2 processing batch {batch_num}/{total_batches}")
    ...
    def _get_model(self) -> SentenceTransformer:
        ...
        with self._model_lock:
            ...
            logger.info(f"Loading sentence-transformers/all-mpnet-base-v2 model: {self.config.model_name}")
            ...
            try:
                ...
                if self.config.device is None or self.config.device == "auto":
                    if torch.backends.mps.is_available() and self.config.enable_mps:
                        ...
                        logger.info("Using Metal Performance Shaders (MPS) for all-mpnet-base-v2")
                    ...
                    if self.config.device in valid_devices:
                        ...
                        logger.warning(f"Invalid device '{self.config.device}', falling back to CPU")
                        self._device = "cpu"
                ...
                if self.config.use_fp16 and self._device in ["cuda", "mps"]:
                    try:
                        if hasattr(self._model, 'half'):
                            self._model.half()
                            logger.info("fp16 acceleration enabled for all-mpnet-base-v2")
                    except Exception as e:
                ...
                logger.info(f"all-mpnet-base-v2 loaded successfully in {self._load_time:.1f}s (device: {self._device}, dim: {self._embedding_dim})")

                return self._model

            except Exception as e:
                logger.warning(f"Failed to load all-mpnet-base-v2 {self.config.model_name}: {e}")
                ...
                if self.config.fallback_model != self.config.model_name:
                    ...
                    try:
                        ...
                        logger.info(f"Fallback model loaded in {self._load_time:.1f}s (dim: {self._embedding_dim})")
                        return self._model
    ...
    async def warmup(self) -> Dict[str, Any]:
        ...
        try:
            # Test embedding
            test_embedding = await self.embed_query("all-mpnet-base-v2 warmup test query for semantic search")
